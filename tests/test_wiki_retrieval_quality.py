...
class TestWikiIndexExclusion:
    def test_excludes_log_index_and_queries_by_default(self, tmp_path):
        ...
        (wiki_root / "papers" / "paper1.md").write_text(
            "# Paper1\n## Method\nDID approach"
        )
        (wiki_root / "papers" / "paper2.md").write_text(
            "# Paper2\n## Results\nSignificant"
        )
        (wiki_root / "topics" / "topic1.md").write_text(
            "# Topic1\n## Overview\nKey findings"
        )
        (wiki_root / "queries" / "q1.md").write_text(
            "# Query1\n## Answer\nResult"
        )
        ...
        for doc_id in store.docs:
            assert "log" not in doc_id, (
                f"log.md should not be indexed: {doc_id}"
            )
            assert "index" not in doc_id, (
                f"index.md should not be indexed: {doc_id}"
            )
            assert "queries" not in doc_id, (
                f"queries should not be indexed: {doc_id}"
            )
    ...
    def test_allows_papers_topics_methods_comparisons(self, tmp_path):
        ...
        for d in ("papers", "topics", "methods", "comparisons"):
            ...
            (wiki_root / d / f"{d}_page.md").write_text(
                f"# {d}\n## Content\nBody"
            )
        ...
        assert store.count() == 8, (
            f"Expected 8 sections (4 pages × 2 sections), got {store.count()}"
        )
    ...
    def test_section_zero_has_no_yaml_leftover(self, tmp_path):
        ...
        for doc_id, payload in store.docs.items():
            ...
            assert not text.startswith("---"), (
                f"Section starts with YAML: {doc_id}"
            )
...
class TestWikiPostprocessing:
    ...
    def test_penalizes_section_zero(self):
        ...
        s0_score = [
            r["score"]
            for r in processed
            if r["metadata"]["section"] == 0
        ][0]
        s1_score = [
            r["score"]
            for r in processed
            if r["metadata"]["section"] == 1
        ][0]
        # Original: 0.9 vs 0.85. After penalty: 0.828 vs 0.85
        assert s0_score < s1_score, (
            "Section 0 should be penalized below section 1"
        )
    ...
    def test_hybrid_pdf_results_are_deduped_by_item_key(self):
        ...
        key1_count = sum(
            1 for r in deduped if r["metadata"]["item_key"] == "key1"
        )
    ...
    def test_pdf_dedup_preserves_highest_scores(self):
        ...
        key1_scores = [
            r["score"]
            for r in deduped
            if r["metadata"]["item_key"] == "key1"
        ]
        assert sorted(key1_scores, reverse=True) == key1_scores, (
...
class TestVectorStoreDeletion:
    def _build_wiki_collection(self, store):
        ...
        store.add_chunks(
            ...
            metadatas=[
                {
                    "page_path": "/wiki/papers/article1.md",
                    "page_type": "papers",
                    "section": 0,
                },
                {
        ...
        store.add_chunks(
            ...
            metadatas=[
                {
                    "page_path": "/wiki/topics/topic2.md",
                    "page_type": "topics",
                    "section": 0,
                },
            ],
